...
function init() {
  time.innerText = timerLimit;
  timeName.innerText = "Time";
  cw.innerText = 0;
  cwName.innerText = "WPM";

  words = randomWords(difficulty);
  renderWords();
...
function startTest() {
  ...
  secondsInterval = setInterval(() => {
    ...
    // Update live WPM
    let timeElapsed = timerLimit - timeRemaining;
    if (timeElapsed > 0) {
      let liveWPM = Math.round((correctKeystrokes / 5) / (timeElapsed / 60));
      cw.innerText = liveWPM;
    }
...
function endTest() {
  ...
  let timeElapsed = timerLimit - timeRemaining;
  let finalWPM = Math.round((correctKeystrokes / 5) / (timerLimit / 60));
  let accuracy = totalKeystrokes === 0 ? 0 : Math.round((correctKeystrokes / totalKeystrokes) * 100);

  time.innerText = accuracy + "%";
  timeName.innerText = "PA";

  cw.innerText = finalWPM;
  cwName.innerText = "WPM";
...
inputItem.addEventListener('keydown', (e) => {
  ...
  if (!isTestActive && e.key.length === 1 && !e.ctrlKey && !e.metaKey) {
    startTest();
  }
  ...
  if (e.key === ' ') {
    e.preventDefault();
    if (currentLetterIndex > 0 || currentWordEl.classList.contains('error')) {
        // Check if current word is completed correctly
        let isWordCorrect = true;
        let typedLetters = currentWordEl.querySelectorAll('.letter.correct, .letter.incorrect, .letter.extra');

        if (typedLetters.length !== letterEls.length) {
            isWordCorrect = false;
        } else {
            for(let l of typedLetters) {
        ...
        // Load more words if we are near the end
        if (currentWordIndex > words.length - 20) {
            let moreWords = randomWords(difficulty);
    ...
    if (currentLetterIndex > 0) {
      // Delete within word
      currentLetterIndex--;
      let letterToClear = letterEls[currentLetterIndex];

      if (letterToClear.classList.contains('extra')) {
          letterToClear.remove();
      } else {
          letterToClear.classList.remove('correct', 'incorrect');
      }
    } else if (currentWordIndex > 0) {
      // Go back to previous word if it has an error
      let prevWordEl = wordEls[currentWordIndex - 1];
      if (prevWordEl.classList.contains('error')) {
          currentWordIndex--;
          currentWordEl.classList.remove('error');
          let prevLetterEls = prevWordEl.querySelectorAll('.letter');

          // Find the last typed index in the previous word
          let lastTyped = 0;
          for (let i = 0; i < prevLetterEls.length; i++) {
              if (prevLetterEls[i].classList.contains('correct') || prevLetterEls[i].classList.contains('incorrect') || prevLetterEls[i].classList.contains('extra')) {
                  lastTyped = i + 1;
              }
          }
    ...
    if (currentLetterIndex < letterEls.length) {
      // Type existing letter
      let letterEl = letterEls[currentLetterIndex];
      let expectedChar = letterEl.innerText;
      ...
      // Type extra letter
      if (currentLetterIndex < letterEls.length + 10) { // Limit extra letters
...
function updateCursorPosition() {
  ...
  // Handle scrolling
  if (currentWordEl.offsetTop > textDisplay.offsetTop + 40) {
      let offset = currentWordEl.offsetTop - textDisplay.offsetTop;
  ...
  if (currentLetterIndex > 0) {
      let letterEls = currentWordEl.querySelectorAll('.letter');
      let targetLetter = letterEls[currentLetterIndex - 1];
      x = targetLetter.offsetLeft + targetLetter.offsetWidth;
      y = targetLetter.offsetTop;
  } else {
      x = currentWordEl.offsetLeft;
      y = currentWordEl.offsetTop;
  }

  cursor.style.left = x + 'px';
  cursor.style.top = y + 4 + 'px'; // adjust slightly for vertical centering
}
...
function randomWords(diff){
  ...
  let wordArray = diff == 1 ? basicWords : topWords;
  let selectedWords = [];
  for(let i=0; i<60; i++){
